...
def split_cluster_acc_v2_new(y_true, y_pred, mask):
    ...
    total_old_to_old_instances = 0
    total_old_to_new_instances = 0
    for i in old_classes_gt:
        old_acc += w[ind_map[i], i]
        total_old_instances += sum(w[:, i])
        total_old_to_old_instances += sum(w[np.array(list(old_classes_gt)), i])
        total_old_to_old_instances -= w[ind_map[i], i]
        total_old_to_new_instances += sum(w[np.array(list(new_classes_gt)), i])
    ...
    total_new_to_old_instances = 0
    total_new_to_new_instances = 0
    for i in new_classes_gt:
        new_acc += w[ind_map[i], i]
        total_new_instances += sum(w[:, i])
        total_new_to_new_instances += sum(w[np.array(list(new_classes_gt)), i])
        total_new_to_new_instances -= w[ind_map[i], i]
        total_new_to_old_instances += sum(w[np.array(list(old_classes_gt)), i])

    new_acc /= total_new_instances

    # calculate bias matrix
    print('old to old error:', total_old_to_old_instances/total_old_instances, 'old to new error:'
          , total_old_to_new_instances/total_old_instances, 'correct', old_acc)
    print('new to new error:', total_new_to_new_instances / total_new_instances, 'new to old error:',
          total_new_to_old_instances / total_new_instances, 'correct', new_acc)
